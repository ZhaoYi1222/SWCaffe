...
namespace caffe {
  ...
  template <typename Dtype>
    void BaseConvolutionLayer<Dtype>::forward_batchsize_cpu_gemm(const int num,const int offset,const Dtype* input,
       ...
       for(int n=0;n<num;n++)
          forward_cpu_gemm(input+n*offset,weights, output+n*output_offset_*group_,skip_im2col) ;
       return;
      ...
      int size = std::min(32,num);
      assert(num % size == 0);
      Dtype *tmp_col_buff = NULL;
      tmp_col_buff = (Dtype*)malloc(size*group_*col_offset_*sizeof(Dtype));
      assert(tmp_col_buff != NULL);
      Dtype * output_buff = (Dtype*)malloc(size*group_*output_offset_*sizeof(Dtype));
      assert(output_buff != NULL);
      for(int n = 0;n< num;n+=size)
      {
        if (!is_1x1_) {
          if (!skip_im2col) {
           conv_im2col_batchsize_cpu(size,group_,input+n*offset, tmp_col_buff);
          }
          col_buff = tmp_col_buff ;
        }
        for (int g = 0; g < group_; ++g) {//reoder col_buff and output
          caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,conv_out_channels_ /
              group_, size*conv_out_spatial_dim_, kernel_dim_,
              (Dtype)1., weights + weight_offset_ * g, col_buff + size*col_offset_* g,
              (Dtype)0., output_buff + size*output_offset_ * g);
        }

       if(typeid(float) == typeid(Dtype)){
         sw_gemm_col2im_f((float*)(output + n*output_offset_*group_),(float*)output_buff,conv_out_channels_,conv_out_spatial_dim_,size);
       }
       else{
         sw_gemm_col2im_d((double*)(output + n*output_offset_*group_),(double*)output_buff,conv_out_channels_,conv_out_spatial_dim_,size);
       }
      }
      free(output_buff);
      free(tmp_col_buff);
  ...
  template <typename Dtype>
    ...
    struct timeval ts, te;
    gettimeofday(&ts, NULL);
    printf("num=%d num_output=%d out_spatial_dim_=%d dim=%d %d\n",num,num_output_,out_spatial_dim_,top_dim_,num_output_*out_spatial_dim_);
#endif
      if(typeid(float) == typeid(Dtype))
      {
         sw_bias_f(num,num_output_,out_spatial_dim_,(float*)bias,(float*)bias_multiplier_.cpu_data(),(float*)output);
      }
      else
      {
         sw_bias_d(num,num_output_,out_spatial_dim_,(double*)bias,(double*)bias_multiplier_.cpu_data(),(double*)output);
      }
#ifdef DEBUG_VERBOSE_3
    gettimeofday(&te, NULL);
  ...
  template <typename Dtype>
    void BaseConvolutionLayer<Dtype>::backward_batchsize_cpu_gemm(const int num,const int offset,const Dtype* output,
      ...
      for(int n=0;n<num;n++)
          backward_cpu_gemm(output+n*output_offset_*group_, weights, input+n*offset);
     return;
#ifdef DEBUG_VERBOSE_3
      struct timeval ts, te;
      gettimeofday(&ts, NULL);
      ...
      for(int n=0;n<num;n += size){
        if(typeid(float) == typeid(Dtype)){
          sw_gemm_im2col_f((float*)output_buff,(float*)(output + n*group_*output_offset_),conv_out_channels_,conv_out_spatial_dim_,size,group_);
        }
        else{
          sw_gemm_im2col_d((double*)output_buff,(double*)(output + n*group_*output_offset_),conv_out_channels_,conv_out_spatial_dim_,size,group_);
        }
        for (int g = 0; g < group_; ++g) {//reoder col_buff and output
          caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kernel_dim_,
              size*conv_out_spatial_dim_, conv_out_channels_ / group_,
              (Dtype)1., weights + weight_offset_ * g, output_buff + size*output_offset_ * g,
              (Dtype)0., col_buff + size*col_offset_ * g);
        }
        if (!is_1x1_) {
          conv_col2im_batchsize_cpu(size,group_,col_buff, input + n * offset);
        }
      }
      free(output_buff);
      free(tmp_col_buff);
  ...
  template <typename Dtype>
    void BaseConvolutionLayer<Dtype>::batchsize_weight_cpu_gemm(const int num,const int offset,const Dtype* input,
     ...
     for(int n=0;n<num;n++) 
       weight_cpu_gemm(input+n*offset,output+n*output_offset_*group_,weights);
     return;
#ifdef DEBUG_VERBOSE_3
      struct timeval ts, te;
      gettimeofday(&ts, NULL);
      ...
      for(int n=0;n<num;n += size){
        ...
        for (int g = 0; g < group_; ++g) {//reoder col_buff 
          caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
              kernel_dim_, size*conv_out_spatial_dim_,
              (Dtype)1., output + n*group_*output_offset_ + size*output_offset_ * g, col_buff + size*col_offset_ * g,
