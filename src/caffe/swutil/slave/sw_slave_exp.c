...
typedef struct expPara_st {
  void *src;
  void *dst;
  int count;
}expPara;

void sw_slave_exp_d(expPara *para) {
  ...
  for(off = 0; off+BUFFSIZE-1<local_count; off+=BUFFSIZE)
  {
    ...
    for(i=0; i<BUFFSIZE; i++) local_dst[i] = exp(local_src[i]);
  ...
  if(off<local_count) {
    ...
    for(i=0; i<local_count-off; i++) local_dst[i] = exp(local_src[i]);
...
void sw_slave_exp_f(expPara *para) {
