...
/*
void sw_slave_axpby_d(axpbyPara *para) {
  ...
  double beta  = para->beta.d;
  SIMDTYPED valpha = alpha;
  SIMDTYPED vbeta = beta;
  ...
  for(off = 0; off+BUFFSIZE-1<local_count; off+=BUFFSIZE)
  {
    ...
    dma(dma_get_src, (long)(dst_ptr+off), (long)(local_dst));
    dma_wait(&replyget, 2); replyget = 0;

    for(i=0; i<BUFFSIZE; i+=SIMDSIZE) {
      simd_load(vsrc,&local_src[i]);
      simd_load(vdst,&local_dst[i]);
      vdst = vbeta * vdst;
      vdst = simd_vmad(vsrc,valpha,vdst); // 
  ...
  if(off<local_count) {
    ...
    for(;i<local_count-off;i++) {
      local_dst[i]=local_src[i]*alpha+local_dst[i]*beta;
    }
...
void sw_slave_axpby_f(axpbyPara *para) {
  ...
  float beta  = para->beta.f;
  SIMDTYPEF valpha = alpha;
  SIMDTYPEF vbeta = beta;
  volatile int replyget=0, replyput=0;
  int i,off;
  // DMA settings
  //dma_desc dma_get_src, dma_put_dst;
  ...
  for(off = 0; off+BUFFSIZE-1<local_count; off+=BUFFSIZE)
  {
    ...
    for(i=0; i<BUFFSIZE; i+=SIMDSIZE) {
      ...
      simd_load(vdst,&local_dst[i]);
      vdst = vbeta * vdst;
      vdst = simd_vmas(vsrc,valpha,vdst); // 
      simd_store(vdst,&local_dst[i]);
    }
