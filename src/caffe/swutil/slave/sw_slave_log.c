...
typedef struct logPara_st {
  void *src;
  void *dst;
  int count;
}logPara;

void sw_slave_log_d(logPara *para) {
  ...
  for(off = 0; off+BUFFSIZE-1<local_count; off+=BUFFSIZE)
  {
    ...
    for(i=0; i<BUFFSIZE; i++) local_dst[i] = log(local_src[i]);
  ...
  if(off<local_count) {
    ...
    for(i=0; i<local_count-off; i++) local_dst[i] = log(local_src[i]);
...
void sw_slave_log_f(logPara *para) {
