...
void sw_slave_pow_d(powPara *para) {
  ...
  for(off = 0; off+BUFFSIZE-1<local_count; off+=BUFFSIZE)
  {
    ...
    for(i=0; i<BUFFSIZE; i++) local_dst[i] = pow(local_src[i],val);
  ...
  if(off<local_count) {
    ...
    for(i=0; i<local_count-off; i++) local_dst[i] = pow(local_src[i],val);
...
void sw_slave_pow_f(powPara *para) {
