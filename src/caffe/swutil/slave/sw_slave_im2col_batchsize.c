...
void sw_im2col_batchsize_large_stride_f(Im2colPara *para) {
#define Type float
...
#define SIMDSIZE 4
  ...
  int local_row_size = (2*para->pad_h + para->height) * count / 64
               + (id< ((2*para->pad_h + para->height) * count % 64));
  ...
  int group_offset = out_channel_size*channels*num/group,g,row;
  int M = kernel_w*kernel_h*channels/group,n;
  ...
  for(ir=row_start;ir<row_end;++ir) {
    ...
    n = (ir+pad_h)/((height+2*pad_h)*channels);
    channel = ((ir+pad_h)/(height+2*pad_h))%channels;
    inoff = channel*width*height + n * channels * width *height;
    // the row is pad
    ...
    // put data by dma
    outoff = num*out_channel_size*channel + n *output_w*output_h;
    for(ic=0;ic<kernel_w;++ic) {
      ...
      for(k=0;k<kernel_h;++k) {
        ...
        g = output_row / M;
        dma( dma_put_col,
            (long)(output_ptr+row*(output_w*output_h*num)+output_col+outoff + g*group_offset),
            (long)(local_outbuff));
...
// Precondition: no dilations. float data type
void sw_im2col_batchsize_large_stride_d(Im2colPara *para) {
#define Type double
...
void sw_im2col_batchsize_large_f(Im2colPara *para) {
#define Type float
#define SIMDType floatv4
#define SIMDSIZE 4
  int pad_h = para->pad_h;
  ...
  for(ir=row_start;ir<row_end;++ir) {
    ...
    // put data by dma
    outoff = num*out_channel_size*channel + n *output_w*output_h;
    //outoff = out_channel_size*channel +num*out_channel_size*channels;
    for(ic=0;ic<kernel_w;++ic) {
...
void sw_im2col_batchsize_large_d(Im2colPara *para) {
#define Type double
#define SIMDType doublev4
#define SIMDSIZE 4
  int pad_h = para->pad_h;
...
// Precondition: no stride and dilations. float data type
void sw_col2im_batchsize_large_f(Im2colPara *para) {
#define Type float
#define SIMDType floatv4
#define SIMDSIZE 4
  int pad_h = para->pad_h;
  ...
  int id = athread_get_id(-1);
  // number of rows of <id> slave core.
  int local_row_size = (2*pad_h + height) * count / 64
               + (id< ((2*pad_h + height) * count % 64));
  // start row index of <id> slave core.
  int row_start= -pad_h+
                  id*((2*pad_h + height)*count/64)
               + id*(id<((2*pad_h + height)*count%64));
  int row_end = row_start+local_row_size; // row_start<= ir < row_end)
  ...
  if(max_batch_size > height) max_batch_size = height;
//#define __SINGLE_BATCH
  ...
  // begin im2col
  int last_index, ik, ib, iout,n;
  for(n=0;n<num;n++){
   ir = row_start;
   while(ir<row_end) {
    input_row = ((ir+pad_h)%(height+2*pad_h))-pad_h;
    channel = (ir+pad_h)/(height+2*pad_h);
    outoff = channel*channel_size + n*channel_size*channels;
    inoff= num*channel*out_channel_size + n*output_w*output_h;
    // the row is pad
    if(!((unsigned)input_row<(unsigned)height)) {
      // it is padding, skip
      ++ir;
      ...
      //dma_set_bsize(&dma_get_col,output_w*sizeof(Type));
      //dma_set_stepsize(&dma_get_col,(num*output_w*output_h - output_w)*sizeof(Type));
      // put data by dma
      for(ic=0;ic<kernel_w;++ic) {
        for(k=0;k<kernel_h;++k) {
          // put output_w size from local_buffer(ic) to 
          ...
          if(output_col>=output_w*output_h) continue; // out of range
          dma( dma_get_col,
              (long)(input_ptr+output_row*(num*output_w*output_h)+output_col+inoff),
              (long)(local_buffer));
...
// Precondition: no stride and dilations. double data type
void sw_col2im_batchsize_large_d(Im2colPara *para) {
#define Type double
#define SIMDType doublev4
#define SIMDSIZE 4
  int pad_h = para->pad_h;
...
// Precondition: no dilations. float data type
void sw_col2im_batchsize_large_stride_f(Im2colPara *para) {
#define Type float
#define SIMDType floatv4
#define SIMDSIZE 4
  int pad_h = para->pad_h;
