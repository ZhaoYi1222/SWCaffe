...
#define LDM_MAX (48*1024)

extern SLAVE_FUN(sw_im2col_batchsize_large_stride_f)();
extern SLAVE_FUN(sw_col2im_batchsize_large_stride_f)();
extern SLAVE_FUN(sw_im2col_batchsize_large_f)();
extern SLAVE_FUN(sw_col2im_batchsize_large_f)();
/*extern SLAVE_FUN(sw_im2col_batchsize_large_d)();
extern SLAVE_FUN(sw_im2col_batchsize_large_stride_d)();
extern SLAVE_FUN(sw_col2im_batchsize_large_d)();
*/
...
void swim2col_batchsize_f(const float* data_im, const int num,const int group,const int channels,
  ...
  // check parameter Precondition of sw_im2col_batchsize_large_d
  assert(dilation_h==1);
  assert(dilation_w==1);
  if(stride_h==1 && stride_w==1) {
    assert((width+2*pad_w)*sizeof(float)<LDM_MAX);
    // spawn
    //printf("SPAWN sw_im2col_batchsize_large_f\n");
    athread_spawn(sw_im2col_batchsize_large_f,para);
    athread_join();
    //printf("sw_col2im_batchsize_large_f end\n");
  } else {
    assert(((width+2*pad_w)+(width+2*pad_w-kernel_w)/stride_w+1)*sizeof(float)<LDM_MAX);
#ifdef PRINT_DEBUGINFO
    printf("SPAWN sw_im2col_batchsize_large_stride_f\n");
#endif
    athread_spawn(sw_im2col_batchsize_large_stride_f,para);
    athread_join();
#ifdef PRINT_DEBUGINFO
    printf("sw_im2col_batchsize_large_stride_f end\n");
#endif
  }
...
void swim2col_batchsize_d(const double* data_im, const int num,const int group,const int channels,
  ...
  if(stride_h==1 && stride_w==1) {
    ...
    // spawn
    //athread_spawn(sw_im2col_batchsize_large_d,para);
    //athread_join();
  } else {
    assert(((width+2*pad_w)+(width+2*pad_w-kernel_w)/stride_w+1)*sizeof(double)<LDM_MAX);
#ifdef PRINT_DEBUGINFO
    printf("SPAWN sw_im2col_batchsize_large_stride_d\n");
#endif
    //athread_spawn(sw_im2col_batchsize_large_stride_d,para);
    //athread_join();
#ifdef PRINT_DEBUGINFO
    printf("sw_im2col_batchsize_large_stride_d end\n");
#endif
  }
...
void swcol2im_batchsize_f(const float* data_col, const int num,const int group,const int channels,
  ...
  if(stride_h==1 && stride_w==1) {
    ...
    // spawn
    athread_spawn(sw_col2im_batchsize_large_f,para);
    athread_join();
  } else {
    assert(((width+2*pad_w-kernel_w)/stride_w+1+width)*sizeof(float)<LDM_MAX);
    athread_spawn(sw_col2im_batchsize_large_stride_f,para);
    athread_join();
  }
...
void swcol2im_batchsize_d(const double* data_col, const int num,const int group,const int channels,
  ...
  // spawn
  //athread_spawn(sw_col2im_batchsize_large_d,para);
  //athread_join();
